...
use pairing::{
    bls12_381::{Bls12, Fr, FrRepr, G1, G1Affine, G1Compressed, G2, G2Affine, G2Compressed, Fq12},
...
///Read G1compressed from [u8;48]
fn read_G1(from: &[u8]) -> G1Compressed {
    assert_eq!(from.len(), 48);

    let mut g1_repr = G1Compressed::empty();
    let result = &mut *g1_repr.as_mut();
    result.copy_from_slice(&from);

    g1_repr
}

///Read G2compressed from [u8;96]
fn read_G2(from: &[u8]) -> G2Compressed {
    assert_eq!(from.len(), 96);

    let mut g2_repr = G2Compressed::empty();
    let result = &mut *g2_repr.as_mut();
    result.copy_from_slice(&from);

    g2_repr
}

///Hash a message to G2, n: length of msg
#[no_mangle]
pub extern "system" fn librustzcash_msg_hash(
    ...
    for _ in 0..8 {
        seed.push(h.read_u32::<BigEndian>().expect("This should work"));
    }
    let p: G2 = ChaChaRng::from_seed(&seed).gen();
    let p = p.into_affine().into_compressed().as_ref().to_vec();
...
#[no_mangle]
pub extern "system" fn librustzcash_sk_to_pk(
    sk: *const [c_uchar; 32],
...
#[no_mangle]
pub extern "system" fn librustzcash_sign(
    msg_hash: *const [c_uchar; 96],
    sk: *const [c_uchar; 32],
    result: *mut [c_uchar; 96],
) -> bool {
    let msg_g2_repr = read_G2(unsafe { &(&*msg_hash)[..] });
    let msg_g2 = match msg_g2_repr.into_affine(){
        Ok(p) => p,
        Err(_) => return false,
    };
    let sk_repr = read_le(unsafe { &(&*sk)[..] });
...
#[no_mangle]
pub extern "system" fn librustzcash_verify(
    signature: *const [c_uchar; 96],
    msg_hash: *const [c_uchar; 96],
    pk: *const [c_uchar; 48],
) -> bool {

    let sig_repr = read_G2(unsafe { &(&*signature)[..] });
    ...
    let msg_g2 = match msg_repr.into_affine(){
        Ok(p) => p,
        Err(_) => return false,
    };

    let pk_repr = read_G1(unsafe { &(&*pk)[..] });
    ...
    let b = match Bls12::final_exponentiation(
        &Bls12::miller_loop([
            (&g1.prepare(), &sig_g2.prepare()),
            (&pk_g1.prepare(), &msg_g2.prepare())
        ].iter())){
        Some(b) => b,
        None => return false,
    };
...
pub fn hash_pks(pks: &Vec<G1Affine>) -> Vec<FrRepr> {
    ...
    for i in 0..n{
        pk_hash_preimage[3] = i as u8;
        let mut h1 = Blake2s::new(32);
        h1.update(&pk_hash_preimage);

        let f = read_le(h1.finalize_inplace());
        t.push(f);
    }
...
///Aggregate public keys into one, pks n*48 bytes, results 48 bytes
#[no_mangle]
pub extern "system" fn librustzcash_pk_aggregate(
    ...
    let pk_sum_repr = pk_sum.into_compressed().as_ref().to_vec();

    let result = unsafe { &mut *result };
    result.copy_from_slice(&pk_sum_repr);
...
#[no_mangle]
pub extern "system" fn librustzcash_sig_aggregate(
    msg_hash: *const [c_uchar; 96],
    sigs: *const c_uchar,
    pks: *const c_uchar,
    n: int64_t,
    ...
    let sig_sum = sig_sum.into_affine();
    let sig_sum_repr = sig_sum.into_compressed().as_ref().to_vec();
